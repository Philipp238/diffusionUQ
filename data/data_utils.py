...
def get_data(
    ...
    if dataset_name in ["cifar", "landscape"]:
        error_msg = (
            f"Images (datasets with target values that have shape with length > 1) "
        ...
        if dataset_name == "cifar":
            ...
            dataset = torchvision.datasets.CIFAR10(
                root=dataset_path, transform=transforms, download=True
            )
    ...
    if dataset_name == "housing_prices":
        ...
        images = torch.tensor(housing["target"], dtype=torch.float32).unsqueeze(
            -1
        )  # (20640, 1)
        ...
        dataset = RegressionDataset(
            images=images, labels=labels, standardize=standardize
        )
    elif dataset_name == "concrete":
        from ucimlrepo import fetch_ucirepo

        # fetch dataset
        yacht_hydrodynamics = fetch_ucirepo(
            id=165
        ...
        # fetch dataset
        combined_cycle_power_plant = fetch_ucirepo(
            id=294
        )  # Yacht Hydrodynamics (https://archive.ics.uci.edu/dataset/243/yacht+hydrodynamics)
        ...
        raise NotImplementedError(
            f"The energy dataset has 2 target variables and it seems like our framework has to be adapted for this first."
        )
        ...
        y = x**2
        dataset = RegressionDataset(
            images=y.unsqueeze(-1), labels=x.unsqueeze(-1), standardize=standardize
        )
    ...
    elif dataset_name in ["1D_Advection", "1D_ReacDiff", "1D_Burgers"]:
        train_dataset = PDE1D(
