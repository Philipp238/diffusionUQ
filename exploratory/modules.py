...
class UNet(nn.Module):
    def __init__(self, c_in=3, c_out=3, time_dim=256, device="cuda"):
        ...
        self.up1 = Up(512, 128)  # the input is actually 256 channels, but since it takes x and skip_x as input, we need conv over 512 channels here
        self.sa4 = SelfAttention(128, 16)
        self.up2 = Up(256, 64)   # same as above
        self.sa5 = SelfAttention(64, 32)
        self.up3 = Up(128, 64)   # same as above
...
class MLP_diffusion(nn.Module):
    ...
    class MLPBlock(nn.Module):
        def __init__(self, hidden_dim=128, concat=False):
            super().__init__()
            self.concat = concat
            if self.concat:
        ...
        def forward(self, x_t):
            return self.act(self.ff(x_t))
    
    def __init__(self, target_dim=1, conditioning_dim=None, concat=False, hidden_dim=128, layers=5, device="cuda"):
        super().__init__()
        self.device = device
        ...
        self.blocks = Sequential2Inputs([MLP_diffusion.MLPBlock(hidden_dim, concat=concat) for _ in range(layers)])
...
class MLP(nn.Module):
    ...
    def __init__(self, target_dim=1, conditioning_dim=None, hidden_dim=128, layers=5):
        super().__init__()
        ...
        self.blocks = nn.Sequential(*[MLP.MLPBlock(hidden_dim) for _ in range(layers)])
